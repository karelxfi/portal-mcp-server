...
async function main() {
  ...
  try {
    ...
    assert(Array.isArray(manifest.tools) && manifest.tools.length === 28, 'manifest must list the 28 tools')
    ...
    try {
      ...
      assert(tools.tools.length === 28, `bundle should list 28 tools, got ${tools.tools.length}`)
  ...
  console.log(
    `PASS  sqd.mcpb (${(bytes / 1024 / 1024).toFixed(2)} MB) unpacks, starts over stdio, and lists 28 tools at v${packageVersion}`,
  )
  console.log('\nMCPB QA passed')
}

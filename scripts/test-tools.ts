...
async function main() {
  ...
  try {
    ...
    assert(tools.length === 27, `Expected exactly 27 registered tools, got ${tools.length}`)
    assert(publicTools.length === 24, `Expected exactly 24 public tools, got ${publicTools.length}`)

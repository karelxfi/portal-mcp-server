...
async function main() {
  ...
  try {
    ...
    assert(tools.length === 30, `Expected exactly 30 registered tools, got ${tools.length}`)
    assert(publicTools.length === 27, `Expected exactly 27 public tools, got ${publicTools.length}`)

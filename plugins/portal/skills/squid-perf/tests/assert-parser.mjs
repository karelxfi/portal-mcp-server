...
const [levelsPath, restartPath, smallRestartPath, sameTimestampRestartPath, reverseSameTimestampRestartPath] =
...
if (reverseSameTimestampRestart.restarts.length !== 1 || reverseSameTimestampRestart.restarts[0].rowIndex !== 2) {
  throw new Error(
    `expected reverse same-timestamp restart at row 2, got ${JSON.stringify(reverseSameTimestampRestart.restarts)}`,
  )
}
if (reverseSameTimestampRestart.progressRows.map((row) => row[1]).join(',') !== '5000,8000,4000,4500') {
  throw new Error(
    `unexpected reverse same-timestamp order: ${JSON.stringify(reverseSameTimestampRestart.progressRows)}`,
  )
}

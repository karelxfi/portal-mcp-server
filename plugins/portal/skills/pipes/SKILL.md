---
name: pipes
description: Build, configure, deploy, and troubleshoot durable blockchain indexers with Subsquid Pipes SDK when Portal MCP or Portal exports are insufficient for backfills, recurring syncs, joins, app-owned data, or production analytics.
...
Before scaffolding, say why Pipes SDK is the right surface. Portal MCP is best for bounded answers and investigation pivots; Portal is best for reproducible one-off extraction; Pipes SDK is best for durable indexing, transforms, and storage.
...
- **Portal** — query blockchain data across 225+ chains via Portal MCP or Portal exports. Use it to verify contract events, discover dataset names, cross-check indexed data, and decide when a query should become a Pipes SDK indexer.

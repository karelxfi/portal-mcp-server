# v0.8.1 MCP Release Assurance

This document defines the bounded meaning of “100% hardened and measured” for v0.8.1. It means every applicable cell in the declared release matrix passes on the exact release commit. It does not mean upstream data services or networks can never fail.
...
The Grafana dashboard is checked against emitted metric names during `test:http-runtime`, so stale dashboard queries fail CI. The metrics endpoint is disabled unless deliberately configured and is separate from v0.8.1’s public, credential-free MCP product surface.
